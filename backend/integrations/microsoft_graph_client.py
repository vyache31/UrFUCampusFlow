...
class GraphClient:
    ...
    async def get_provider_user_info(self, headers):
        payload = await self.session.get(
            settings.OAUTH_MICROSOFT_ME_URL, headers=headers
        )
    ...
    async def create_event(self, event_info: dict, headers):
        payload = await self.session.post(
            settings.OAUTH_MICROSOFT_EVENTS_URL, json=event_info, headers=headers
        )
    ...
    async def delete_event(self, event_id: str, headers):
        payload = await self.session.delete(
            f"{settings.OAUTH_MICROSOFT_EVENTS_URL}/{event_id}", headers=headers
        )
    ...
    async def list_calendar_view(self, params: dict, headers):
        payload = await self.session.get(
            settings.OAUTH_MICROSOFT_CALENDAR_VIEW_URL, params=params, headers=headers
        )
    ...
    async def get_series_instances(self, series_id: str, params: dict, headers):
        payload = await self.session.get(
